...
#### R code used for analysis of single-cell RNA sequencing data for Figure 2
...
clusters <- c(2, 16, 18, 21, 22, 23, 25)
cellnamesfilter <- c()
...
set.seed(100)
csce_in <- runTSNE(sce, use_dimred="MNN")

sro_in <- makeSeuratFromSingleCellExperiment(csce_in)
sro_in@data <- logcounts(csce_in)

sro_in <- FindClusters(object = sro_in, reduction.type = "MNN", dims.use = 1:50, resolution = 1.4)

# Identification of markers using scfind
csce_in <- csce_in[!duplicated(rownames(csce_in)), ]
rowData(csce_in)$feature_symbol <- rownames(csce_in)
csce_in <- csce_in[!duplicated(rownames(csce_in)), ]

csce_in$cell_type1 <- sro_in@meta.data$res.1.4

geneIndex <- buildCellTypeIndex(csce_in, "cell_type1")
